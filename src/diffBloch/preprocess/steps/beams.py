...
from diffBloch.core.products import PLAIN_SUM, MosaicAverage, TiltReduction
...
def build_orientation_plans(
    ...
    *,
    ...
    """
    if mosaicity and rocking is None:
    ...
    def geometry(candidate: CandidatePlan) -> tuple[NDArray[np.float64] | None, TiltReduction]:
        ...
        sigma = candidate.mosaicity_degrees
        if sigma is None:
            raise ValueError("mosaicity is enabled but the candidate has no PETS mosaicity value")
        tilts, weights = mosaic_rocking_curve_tilts(
            rocking.integration.semiangle,
            rocking.sampling,
            sigma,
            geometry=rocking.integration.geometry,
        )
        return tilts, MosaicAverage(tuple(float(value) for value in weights), sigma)

    def run(plan: Plan) -> Plan:
        candidates = require_candidate_plans(plan)
        built: tuple[OrientationPlan | CoupledOrientationPlan, ...]
        if coupling is None:
            def build_plain(cp: CandidatePlan) -> OrientationPlan:
                tilts, reduction = geometry(cp)
            ...
            built = tuple(build_plain(cp) for cp in candidates)
        else:
            ...
            def build_one(candidate: CandidatePlan) -> CoupledOrientationPlan:
                tilts, reduction = geometry(candidate)
