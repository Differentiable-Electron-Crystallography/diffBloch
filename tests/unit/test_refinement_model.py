...
Phase 1 introduces ``RefinementModel`` / ``PhysicalStructure`` as a no-behavior-change seam. These
checks pin that the wrapper delegates to the existing physical structure path, including a quartz
fixture objective comparison so refactors cannot silently alter the underlying physics model.
"""
...
def test_physical_structure_wraps_existing_refinement_inputs() -> None:
    params = _params()
    structure = PhysicalStructure(initial=params)
...
def test_build_refinement_model_is_structure_only_adapter() -> None:
    ...
    assert model == RefinementModel(structure=PhysicalStructure(initial=params))
