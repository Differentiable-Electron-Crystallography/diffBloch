...
def test_refine_best_params_can_track_a_selection_engine(tmp_path: Path) -> None:
    ...
    step_events = [e for e in records if e.event_type == "RefinementStep"]
    assert len(step_events) == 8
    assert all(
        e.payload["val_wr2"] is not None and e.payload["val_r_obs"] is not None for e in step_events
    )
    assert all(e.payload["val_n_rotations"] == 1 for e in step_events)


def test_refinement_step_omits_validation_fields_without_a_selection_engine(
    tmp_path: Path,
) -> None:
    ...
    step_events = [e for e in _records(path) if e.event_type == "RefinementStep"]
    assert len(step_events) == 3
    assert all(e.payload["val_wr2"] is None and e.payload["val_r_obs"] is None for e in step_events)
