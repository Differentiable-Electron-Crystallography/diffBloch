...
def test_segment_count_and_covers_match_private(rotation: int) -> None:
    ...
    for k, segment in enumerate(segments):
        assert list(segment.covered_tilt_indices) == [int(t) for t in d[f"seg{k}_cover"]]
    # Covers tile every tilt exactly once, disjoint and contiguous.
    covered = [t for segment in segments for t in segment.covered_tilt_indices]
...
def test_segment_beam_sets_contain_private(rotation: int) -> None:
    ...
    for k, segment in enumerate(segments):
        ...
        assert _as_set(segment.union_hkl) >= _as_set(d[f"seg{k}_hkl"]), f"segment {k} beam set"
