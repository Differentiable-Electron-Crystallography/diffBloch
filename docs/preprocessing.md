...
The orientation fit forks on unit-cell volume, not on a config switch: a **large cell**
(> 1000 Å³) skips the per-trial gather integrity checks (`validate=False`), made sound instead by
`optimize_orientation`'s own coupled coverage guard; a **small cell** takes the exact,
fully-validated path (`validate=True`). This exists because beam count, and with it the O(N³)
eigensolve cost, scales with cell volume -- the large-cell branch avoids validation overhead that
matters once the eigensolve itself dominates.

The predicate reads only the pipeline-invariant grid, so the fork compiles to a flat branch (the
branch taken is fixed per experiment) before the recipe's provenance is locked -- `validate` is
execution-only and never enters the step identity, so a committed checkpoint is unaffected by which
branch produced it. The thickness fit has no such split and always runs the same path regardless of
cell size.
