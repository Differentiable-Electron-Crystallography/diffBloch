...
- [ ] **3 — `core/crystal` + `reciprocal`.** Relocate the already-tested helpers (mechanical move).
- [ ] **4 — `core/constraints` + `adp`.** Cholesky + symmetry-mask bijectors; delete the in-place Uij
  loop.
- [ ] **5 — `core/symmetry::expand_asu`.** Precomputed membership; remove per-step numpy dedup
  (membership-order golden).
- [ ] **6 — `core/scattering`.** From `StructureFactorNet`; vectorise per-atom loops (grid golden).
- [ ] **7 — `core/dynamical` + `solver`.** `build_bloch_system(...) -> BlochSystem` (carries
...
- [ ] **8 — `core/losses` + `products`.** Move loss bodies out; retire `DiffractionDataset` for
  `PatternBatch` / `BlochSolution`.
- [ ] **9 — `params`/`specs` + `engine`.** `RefinableParams` (incl. `b_dose_raw`) → `constrain` →
...
- [ ] **10 — `preprocess/`.** Clean reimplementation of orientation + thickness + numeric-convergence
  behind `engine.fit_*()` / `converge_numerics()`.
- [ ] **11 — `logging` + `app/`.** Pluggable `Logger` (NullLogger default; wandb/CSV/MLflow as
  swappable backends — no vendor SDK in core); thin `cli.py`; pluggable `sweep.py`.
- [ ] **12 — Cleanup.** Delete deprecated adapters; final e2e + full unit run. `RunRef` op-boundary
